...
#pragma warning disable VSTHRD002
        projectScope.StubIdeScope.Substitute.When(m => m
            .FireAndForgetOnBackgroundThread(Arg.Any<Func<CancellationToken, Task>>(), Arg.Any<string>()))
                    .Do(callInfo =>
                    {
                        callInfo.Arg<Func<CancellationToken, Task>>()(projectScope.StubIdeScope.BackgroundTaskTokenSource.Token).Wait();
                    });
#pragma warning restore VSTHRD002
    ...
    public void TriggerDiscoveryUpdatesTheCache()
    {
        ...
        //assert
        sut.BindingRegistryCache.Substitute.Received().Update(Arg.Any<Func<ProjectBindingRegistry, Task<ProjectBindingRegistry>>>());
    ...
    public void DoNotTriggersCacheUpdateOnEventsForTheSameProject(LabeledTestData<Action<Sut>> triggerEvent)
    {
        ...
        //assert
        sut.BindingRegistryCache.Substitute.Received(1).Update(Arg.Any<Func<ProjectBindingRegistry, Task<ProjectBindingRegistry>>>()); // the cache update have to be called only once when the project haven't changed
    ...
    [Fact]
    public void DoNotDiscoverWhenProjectIsNotReqnrollTestProject()
    {
        //arrange
        using var sut = ArrangeSut();
        ...
        //assert
        discovered.Version.Should().Be(1);
        sut.ProjectScope.StubIdeScope.StubLogger.Messages.Should().Contain("Non-Reqnroll test project");
    }
    ...
    public void DoNotDiscoverWhenConfigSourceIsInvalid()
    {
        ...
        discovered.Version.Should().Be(1);
        var expected =
            "Test assembly not found. Please build the project to enable the Reqnroll Visual Studio Extension features.";
