...
public class StepParameterNameGenerator
{
    ...
    public static bool IsSingleWordWithNoNumbers(string value) => Regex.IsMatch(value, @"^[\p{L}]+$");
    ...
    public static string RemoveSurroundingAngleBrackets(string value) => value.Substring(1, value.Length - 2);

    public static string LowerCaseFirstLetter(string value) => value.Substring(0, 1).ToLower() + value.Substring(1);

    public static string UniquelyIdentified(string value, List<string> usedParameterNames, int parmIndex)
    {
        if (usedParameterNames.Contains(value)) return value + parmIndex;
